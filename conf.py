...
class Config:
    ...
    glance use this config for drive django framework running,
    You can set is value or set the same envirment value,
    glance look for config order: file => env => default

    glance使用配置来驱动Django框架的运行，
    你可以在该文件中设置，或者设置同样名称的环境变量,
    glance使用配置的顺序: 文件 => 环境变量 => 默认值
    """
